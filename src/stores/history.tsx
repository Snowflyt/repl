...
const historyStore = create({
  ...
  appendError(error: unknown) {
    ...
    this.history.push(HistoryEntry.ErrorMessage({ value: message }));
  },
...
const encodeHistoryEntryV2 = match<string, HistoryEntry>()({
  ...
  ErrorMessage: ({ value }) => "e" + writeStr(value),
});
  ...
  while (i < data.length) {
    ...
    switch (code) {
      ...
      case "e": {
        const [s, j] = readStr(data, i);
        i = j;
        result.push(HistoryEntry.ErrorMessage({ value: s }));
        break;
      }
      case "j": {
...
const decodeHistoryEntryV1 = match<HistoryEntry, readonly [string, string]>()({
  ...
  "['e', _]": (value) => HistoryEntry.ErrorMessage({ value }),
  "[_, *]": (format) => {
