...
const ProgressBar = React.memo<{ mode: "determinate" | "indeterminate"; value?: number }>(
  function ProgressBar({ mode, value }) {
    if (mode === "determinate") {
      ...
      return (
        <div className="mt-2 h-1.5 w-full overflow-hidden rounded bg-white/10">
          <div
            className="h-full rounded bg-gradient-to-r from-[#9c6bff] via-[#7aa2ff] to-[#6affb0]"
            style={{ width: pct + "%" }}
          />
        </div>
      );
    ...
    return (
      <div className="mt-2 h-1.5 w-full overflow-hidden rounded bg-white/10">
        <div
          className="h-full w-1/3 animate-[slide_1.2s_ease-in-out_infinite] rounded bg-gradient-to-r from-[#9c6bff] via-[#7aa2ff] to-[#6affb0]"
...
const NotificationCenter = React.memo<{
  ...
  return (
    <div
      className={clsx(
        "pointer-events-none fixed z-20 flex max-h-[55vh] w-[26rem] max-w-[calc(100vw-32px)] flex-col gap-2 overflow-hidden",
        className,
      )}
      style={mergedStyle}>
      {ordered.map((n) => (
